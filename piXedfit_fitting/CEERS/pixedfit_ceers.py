...
cat_eazy = fits.open('ceers-full-grizli-v6.0-fix.eazypy.zout.fits')
...
cat_photo = fits.open('ceers-full-grizli-v6.0-fix_phot_apcorr.fits')
# get the catalog data
cat_photo_data = cat_photo[1].data
cat_photo.close()

# check the list of filters and their names from the header information printed above
filter_names = ['f105w','f115w', 'f125w', 'f140w', 'f150w', 'f160w', 'f200w', 'f277w',
...
not_quiescent = [86, 198, 213, 328, 331, 380, 397, 554, 785, 869, 1195, 1235, 1677, 2216, 2450, 3369, 4170, 4374, 4492, 5286, 5332, 5822, 5983, 6281, 6308, 6760, 6905, 7666, 7805, 7955, 8576, 8873, 8901, 9040, 9317, 9585, 10491, 10667, 12116, 12150, 12414, 13668, 13966, 13996, 14875, 14876, 14908, 14963, 15424, 15474, 15795, 16741, 16789, 16971, 17101, 17285, 17436, 17519, 17629, 17852, 18160, 18215, 18520, 18625, 18639, 18849, 21703, 21923, 22158, 22160, 22536, 22719, 22908, 23039, 24327, 24485, 24667, 24939, 25078, 25197, 25199, 25397, 25758, 27042, 27043, 27709, 27725, 27962, 28282, 28348, 28369, 28750, 28760, 29229, 29369, 29455, 30085, 30223, 30568, 30859, 31305, 31423, 31437, 31574, 31688, 32227, 32839, 33227, 33928, 36188, 36480, 36743, 36836, 36930, 38052, 38057, 38400, 39333, 39704, 39785, 40370, 40499, 40500, 40643, 40974, 42607, 42824, 43245, 46140, 46205, 46207, 46282, 46326, 46392, 46556, 47964, 48011, 48164, 48671, 48930, 50450, 50453, 51368, 51427, 51673, 52644, 53380, 53479, 54438, 54798, 55187, 55641, 56031, 56032, 56065, 57405, 57446, 57455, 57651, 57790, 57890, 58047, 58103, 59151, 59219, 59472, 60601, 60670, 61341, 61621, 62421, 62647, 62941, 63090, 63793, 63858, 64661, 64779, 65526, 65853, 65854, 66137, 66195, 66215, 66782, 66890, 66941, 67336, 67623, 67624, 67786, 67888, 68419, 68509, 68516, 68517, 68924, 68978, 69392, 69408, 70103, 70151]
...
filters = ['hst_acs_f435w', 'hst_wfpc3_f606w', 'hst_acs_f606w', 'hst_acs_f814w', 'hst_wfc3_ir_f105w', 'jwst_nircam_f115w', 'hst_wfc3_ir_f125w', 'hst_wfc3_ir_f140w', 'jwst_nircam_f150w', 'hst_wfc3_ir_f160w', 'jwst_nircam_f200w', 'jwst_nircam_f277w', 'jwst_nircam_f356w', 'jwst_nircam_f410m', 'jwst_nircam_f444w']
...
for i in range(2, 7):
    ...
    for ii in range(len(ids_sample_galaxies)):
        ...
        if (z_phot[idx_gal_cat] >= i) and (z_phot[idx_gal_cat] <= i+1):
            ...
            name_out_fits = '%d_ceers_pixedfit_mcmc_%d.fits' % (i,idx_gal_cat+1)
